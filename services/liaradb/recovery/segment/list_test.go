package segment

import (
	"fmt"
	"path"
	"slices"
	"testing"
	"testing/fstest"

	"github.com/liaradb/liaradb/filecache"
	"github.com/liaradb/liaradb/recovery/logpage"
	"github.com/liaradb/liaradb/util/testing/filetesting"
)

func TestList_Open(t *testing.T) {
	t.Parallel()

	dir := "log"
	var count SegmentID = 10

	t.Run("should handle no dir", func(t *testing.T) {
		t.Parallel()

		fsys := createFiles(dir, 0, 0)
		sl := NewList(fsys, dir, 128, 128)

		if err := sl.Open(); err != nil {
			t.Fatal(err)
		}

		names := sl.Names()

		want := createNames(0, 0)
		if !slices.Equal(want, names) {
			t.Errorf("files do not match:\n\t%v,\nexpected:\n\t%v", names, want)
		}
	})

	t.Run("should list segments", func(t *testing.T) {
		t.Parallel()

		fsys := createFiles(dir, 0, count)
		sl := NewList(fsys, dir, 128, 128)

		if err := sl.Open(); err != nil {
			t.Fatal(err)
		}

		names := sl.Names()

		want := createNames(0, count)
		if !slices.Equal(want, names) {
			t.Errorf("files do not match:\n\t%v,\nexpected:\n\t%v", names, want)
		}
	})

	t.Run("should list segments in order", func(t *testing.T) {
		t.Parallel()

		fsys := createFiles(dir, 9998, count)
		sl := NewList(fsys, dir, 128, 128)

		if err := sl.Open(); err != nil {
			t.Fatal(err)
		}

		names := sl.Names()

		want := createNames(9998, count)
		if !slices.Equal(want, names) {
			t.Errorf("files do not match:\n\t%v,\nexpected:\n\t%v", names, want)
		}
	})
}

func TestList_OpenLatestSegment(t *testing.T) {
	t.Parallel()

	dir := "log"

	for message, test := range map[string]struct {
		result SegmentID
		fsys   filecache.FileSystem
	}{
		"should handle no files": {0, filetesting.New(fstest.MapFS{
			fmt.Sprintf("%v/", dir): &fstest.MapFile{},
		})},
		"should handle one file": {1, filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
		})},
		"should handle multiple files": {2, filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
			createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
		})},
	} {
		t.Run(message, func(t *testing.T) {
			t.Parallel()

			sl := NewList(test.fsys, dir, 128, 128)

			f, err := sl.OpenLatestSegment()
			if err != nil {
				t.Fatal(err)
			}

			sn := f.SegmentName()
			if id := sn.ID(); id != test.result {
				t.Errorf("wrong id: %v, expected: %v", id, test.result)
			}

			if f == nil {
				t.Error("file should not be nil")
			}

			if names := sl.Names(); !slices.Contains(names, sn) {
				t.Errorf("list does not contain segment: %v", sn)
			}
		})
	}

	t.Run("should close previous file", func(t *testing.T) {
		t.Parallel()

		dir := "log"

		fsys := filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
			createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
		})
		sl := NewList(fsys, dir, 128, 128)

		f, err := sl.OpenLatestSegment()
		if err != nil {
			t.Fatal(err)
		}

		if _, err = sl.OpenNextSegment(logpage.NewLogSequenceNumber(30)); err != nil {
			t.Fatal(err)
		}

		if _, err := sl.OpenLatestSegment(); err != nil {
			t.Fatal(err)
		}

		if f.IsOpen() {
			t.Error("previous file should be closed")
		}
	})
}

func TestList_IterateFromLSN(t *testing.T) {
	t.Parallel()

	dir := "log"

	sn0 := NewSegmentName(1, logpage.NewLogSequenceNumber(10))
	sn1 := NewSegmentName(2, logpage.NewLogSequenceNumber(20))
	sn2 := NewSegmentName(3, logpage.NewLogSequenceNumber(30))

	for message, c := range map[string]struct {
		skip   bool
		names  []SegmentName
		result []SegmentName
	}{
		"should find a segment name": {
			names:  []SegmentName{sn0, sn1, sn2},
			result: []SegmentName{sn0, sn1, sn2},
		},
	} {
		t.Run(message, func(t *testing.T) {
			t.Parallel()
			if c.skip {
				t.Skip()
			}

			m := fstest.MapFS{}
			for _, sn := range c.names {
				m[createPath(dir, sn)] = &fstest.MapFile{}
			}
			fsys := filetesting.New(m)
			sl := NewList(fsys, dir, 128, 128)

			var n []SegmentName
			for f, err := range sl.IterateFromLSN(logpage.NewLogSequenceNumber(10)) {
				if err != nil {
					t.Fatal(err)
				}

				m, _ := f.Stat()
				n = append(n, ParseSegmentName(m.Name()))
			}

			if !slices.Equal(c.result, n) {
				t.Error("names do not match")
			}
		})
	}
}

func TestList_OpenSegmentForLSN(t *testing.T) {
	t.Parallel()

	dir := "log"

	fsys := filetesting.New(fstest.MapFS{
		createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
		createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
	})
	sl := NewList(fsys, dir, 128, 128)

	for message, test := range map[string]struct {
		search uint64
		found  bool
		result uint64
	}{
		"should not find low value": {1, false, 0},
		"should find exact value":   {10, true, 10},
		"should find middle value":  {15, true, 10},
		"should find high value":    {50, true, 20},
	} {
		t.Run(message, func(t *testing.T) {
			t.Parallel()

			f, err := sl.OpenSegmentForLSN(logpage.NewLogSequenceNumber(test.search))
			if test.found {
				if err != nil {
					if err == ErrNoSegmentFile {
						t.Error("should find log sequence number")
					} else {
						t.Error(err)
					}
				}

				result := logpage.NewLogSequenceNumber(test.result)
				if lsn := f.SegmentName().LogSequenceNumber(); lsn != result {
					t.Errorf("wrong log sequence number: %v, expected: %v", lsn, result)
				}
				if f == nil {
					t.Error("file should not be nil")
				}
			} else {
				if err != ErrNoSegmentFile {
					t.Error("should not find log sequence number")
				}
				if f != nil {
					t.Error("file should be nil")
				}
			}
		})
	}

	t.Run("should close previous file", func(t *testing.T) {
		t.Parallel()

		fsys := filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
			createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
		})
		sl := NewList(fsys, dir, 128, 128)

		f, err := sl.OpenSegmentForLSN(logpage.NewLogSequenceNumber(10))
		if err != nil {
			t.Fatal(err)
		}

		if _, err := sl.OpenSegmentForLSN(logpage.NewLogSequenceNumber(20)); err != nil {
			t.Fatal(err)
		}

		if f.IsOpen() {
			t.Error("previous file should be closed")
		}
	})
}

func TestList_OpenNextSegment(t *testing.T) {
	t.Parallel()

	dir := "log"

	t.Run("should open next segment", func(t *testing.T) {
		fsys := filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
			createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
		})
		sl := NewList(fsys, dir, 128, 128)

		f, err := sl.OpenNextSegment(logpage.NewLogSequenceNumber(30))
		if err != nil {
			t.Fatal(err)
		}

		sn := f.SegmentName()
		if id := sn.ID(); id != 3 {
			t.Errorf("wrong id: %v, expected: %v", id, 3)
		}

		if f == nil {
			t.Error("file should not be nil")
		}

		if names := sl.Names(); len(names) <= 3 && !slices.Contains(names, sn) {
			t.Errorf("list does not contain segment: %v", sn)
		}

		// Open again to verify
		if err := sl.Open(); err != nil {
			t.Fatal(err)
		}

		if names := sl.Names(); len(names) <= 3 && !slices.Contains(names, sn) {
			t.Errorf("list does not contain segment: %v", sn)
		}
	})

	t.Run("should close previous file", func(t *testing.T) {
		t.Parallel()

		fsys := filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
		})
		sl := NewList(fsys, dir, 128, 128)

		f, err := sl.OpenNextSegment(logpage.NewLogSequenceNumber(20))
		if err != nil {
			t.Fatal(err)
		}

		if _, err := sl.OpenNextSegment(logpage.NewLogSequenceNumber(30)); err != nil {
			t.Fatal(err)
		}

		if f.IsOpen() {
			t.Error("previous file should be closed")
		}
	})
}

func TestList_OpenSegmentBeforeLSN(t *testing.T) {
	t.Parallel()

	dir := "log"

	t.Run("should open segment", func(t *testing.T) {
		fsys := filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
			createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
			createPath(dir, NewSegmentName(3, logpage.NewLogSequenceNumber(30))): {},
		})
		sl := NewList(fsys, dir, 128, 128)

		f, err := sl.OpenSegmentBeforeLSN(logpage.NewLogSequenceNumber(30))
		if err != nil {
			t.Fatal(err)
		}

		sn := f.SegmentName()
		if id := sn.ID(); id != 2 {
			t.Errorf("wrong id: %v, expected: %v", id, 2)
		}

		if f == nil {
			t.Error("file should not be nil")
		}

		if names := sl.Names(); !slices.Contains(names, sn) {
			t.Errorf("list does not contain segment: %v", sn)
		}
	})

	t.Run("should close previous file", func(t *testing.T) {
		t.Parallel()

		fsys := filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
			createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
			createPath(dir, NewSegmentName(3, logpage.NewLogSequenceNumber(30))): {},
		})
		sl := NewList(fsys, dir, 128, 128)

		f, err := sl.OpenSegmentBeforeLSN(logpage.NewLogSequenceNumber(30))
		if err != nil {
			t.Fatal(err)
		}

		if _, err := sl.OpenSegmentBeforeLSN(logpage.NewLogSequenceNumber(20)); err != nil {
			t.Fatal(err)
		}

		if f.IsOpen() {
			t.Error("previous file should be closed")
		}
	})

	t.Run("should not open invalid segment", func(t *testing.T) {
		t.Parallel()

		fsys := filetesting.New(fstest.MapFS{
			createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
			createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
			createPath(dir, NewSegmentName(3, logpage.NewLogSequenceNumber(30))): {},
		})
		sl := NewList(fsys, dir, 128, 128)

		if _, err := sl.OpenSegmentBeforeLSN(logpage.NewLogSequenceNumber(10)); err == nil {
			t.Error("should not open file")
		}
	})
}

func TestList_RemoveSegmentBeforeLSN(t *testing.T) {
	t.Parallel()

	dir := "log"

	fsys := filetesting.New(fstest.MapFS{
		createPath(dir, NewSegmentName(1, logpage.NewLogSequenceNumber(10))): {},
		createPath(dir, NewSegmentName(2, logpage.NewLogSequenceNumber(20))): {},
	})
	sl := NewList(fsys, dir, 128, 128)

	if err := sl.RemoveSegmentBeforeLSN(logpage.NewLogSequenceNumber(20)); err != nil {
		t.Fatal(err)
	}

	if names := sl.Names(); !slices.Equal(names, []SegmentName{NewSegmentName(2, logpage.NewLogSequenceNumber(20))}) {
		t.Errorf("list is incorrect: %v", names)
	}

	// Open again to verify
	if err := sl.Open(); err != nil {
		t.Fatal(err)
	}

	if names := sl.Names(); !slices.Equal(names, []SegmentName{NewSegmentName(2, logpage.NewLogSequenceNumber(20))}) {
		t.Errorf("list is incorrect: %v", names)
	}
}

func TestList_Reverse(t *testing.T) {
	t.Parallel()

	dir := "log"

	sn0 := NewSegmentName(1, logpage.NewLogSequenceNumber(10))
	sn1 := NewSegmentName(2, logpage.NewLogSequenceNumber(20))
	sn2 := NewSegmentName(3, logpage.NewLogSequenceNumber(30))
	names := []SegmentName{sn0, sn1, sn2}
	slices.Reverse(names)
	fsys := filetesting.New(fstest.MapFS{
		createPath(dir, sn0): {},
		createPath(dir, sn1): {},
		createPath(dir, sn2): {},
	})
	sl := NewList(fsys, dir, 128, 128)

	c := 0
	n := make([]SegmentName, 0, 3)
	for f, err := range sl.Reverse() {
		if err != nil {
			t.Fatal(err)
		}

		m, _ := f.Stat()
		n = append(n, ParseSegmentName(m.Name()))
		c++
	}
	if c != 3 {
		t.Errorf("incorrect count: %v, expected: %v", c, 3)
	}
	if !slices.Equal(names, n) {
		t.Error("names do not match")
	}
}

func createNames(start SegmentID, count SegmentID) []SegmentName {
	names := make([]SegmentName, 0, count)
	for i := range count {
		names = append(names, NewSegmentName(start+i, logpage.NewLogSequenceNumber(0)))
	}
	return names
}

func createFiles(dir string, start SegmentID, count SegmentID) filecache.FileSystem {
	mapfs := fstest.MapFS{}
	for i := range count {
		mapfs[createPath(dir, NewSegmentName(start+i, logpage.NewLogSequenceNumber(0)))] = &fstest.MapFile{}
	}
	return filetesting.New(mapfs)
}

func createPath(dir string, sn SegmentName) string {
	return path.Join(dir, sn.String())
}
