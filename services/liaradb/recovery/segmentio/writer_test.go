...
func testWriter_Append(t *testing.T) {
	...
	if _, err := sw.AppendRecord(rec); err != nil {
		t.Error(err)
	}
}
...
func testWriter_SeekTail(t *testing.T) {
	...
	if _, err := sw2.AppendRecord(rec); err != nil {
		t.Fatal(err)
	}
...
func TestWriter_Flush(t *testing.T) {
	...
	t.Run("should write to multiple pages", func(t *testing.T) {
		...
		synctest.Test(t, func(t *testing.T) {
			...
			for range count - 1 {
				if _, err := l.AppendRecord(rec); err != nil {
					t.Fatal(err)
				}
			}
	...
	t.Run("should return error if appending beyond maximum", func(t *testing.T) {
		...
		synctest.Test(t, func(t *testing.T) {
			...
			if _, err := l.AppendRecord(rec); err != raw.ErrInsufficientSpace {
