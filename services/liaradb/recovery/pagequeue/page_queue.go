...
func (pq *PageQueue) flushCurrent(ctx context.Context, lsn record.LogSequenceNumber, current *page.LogPage) {
	if pq.flushed {
...
func (pq *PageQueue) replaceCurrent(lsn record.LogSequenceNumber, p *page.LogPage) {
	pq.lsn = lsn
	pq.current = p
	pq.flushed = false
}
